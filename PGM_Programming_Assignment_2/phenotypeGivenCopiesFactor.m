...
for p = 1:numPhenotypes
    for a1 = 1:numAlleles
        for a2 = 1:numAlleles
            i = AssignmentToIndex([p a1 a2], phenotypeFactor.card);
            g = allelesToGenotypes(a1, a2);
            if p == 1
                phenotypeFactor.val(i) = alphaList(g);
            else
                phenotypeFactor.val(i) = 1 - alphaList(g);
            end
        end
    end
end
